...
public class carRotate : MonoBehaviour {
		...
		if (!gameover) {
			...
			if (gameObject.transform.position.z + 0 < collisionInfo.collider.transform.position.z) {
				...
				if (collisionInfo.collider.rigidbody != null) {	
					...
					if(!collisionInfo.collider.name.Equals("BottomBlock"))
	...
	void Update () {
		...
		if (fire) {
			...
			if(fmiss==null)
				fmiss=Instantiate (missile, this.transform.position+new Vector3(0,1.6f,-2), Quaternion.Euler(Vector3.zero)) as GameObject;
